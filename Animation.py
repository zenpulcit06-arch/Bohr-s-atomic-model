...
te_max = np.max(te)
te_min = np.min(te)
plt.yticks(np.arange(te_min,te_max,0.2))
ax2.set_ylim(te_min - 0.05, te_max + 0.05)
...
ani = FuncAnimation(fig,update,frames=range(0,len(df),20),interval=30,blit = True, repeat = False)
